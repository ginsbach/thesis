...
\end{figure}
    ...
    \Cref{subsec:impl} uses this to derive an efficient implementation.

    \Cref{theo:theo2} introduces backtracking solutions for conjunction
    constraint problems.
    These are constructed in the obvious way, by taking the intersection set of
...
\end{figure}
    ...
    It is constructed entirely according to
    \Cref{theo:theo1,theo:theo2,theo:theo4}.
    For example, the condition
    ``$(x_\text{step},x_\text{update})\in DFG_\mathcal F^*$'' is the
    element-of constraint problem $E_S$ with
    $S=\{x\in\mathbb N^{\{\text{step},\text{update}\}}\mid (x_\text{step},x_\text{update})\in DFG_\mathcal F^*\}$.
    Following \Cref{theo:theo1}, the enumeration $(\text{update},\text{step})$
    gives
    \begin{align*}
    ...
    \end{align*}
    This backtracking solution is now extended to $V$ according to
    \Cref{theo:theo4}, meaning that $B_1[E_S]$ features in
    $B_1[C]$ and $B_2[E_S]$ features in $B_2[C]$, but $B_3[C]$ is entirely
    independent of $E_S$.

    Finally, the bottom right of the figure shows the backtracking algorithm
    applied to the SSA model from \Cref{fig:derivemaths}.
