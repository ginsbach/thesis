...
\begin{figure*}[p]
    \begin{definition}{Partial Constraint Problems}{partconstr}
        Assume an enumeration $V=\{v_1,\dots,v_n\}$.

        Then the {\em partial set of varibles} $V_k$ is
        defined as
        \begin{align*}
            V_0:={}&\{\}\quad\\
        ...
        and the {\em restricted constraint predicate}
        ...
        Finally, the {\em partial constraint problem} $C_k=(V_k,P_k)$ is defined
        by the predicate $P_k$ with
        \begin{align*}
            P_0:={}&R_0\\
        ...
        \end{align*}
    \end{definition}
    \begin{definition}{Approximate Partial Constraint Problems}{approxconstr}
        An {\em approximate partial constraint problem}
        $\tilde C_k=(V_k,\tilde P_k)$ is a constraint problem, for which the
        predicate fulfills the following property.
        \begin{align*}
            P_k(x,M)=true\implies\tilde P_k(x,M)=true
        \end{align*}

        The approximation error is defines as the number of $x\in\mathbb N^V$
        such that
        \begin{align*}
            \tilde P_k(x,M)=true\mathrel\land P_k(x,M)=false.
        \end{align*}
    \end{definition}
\end{figure*}
    ...
    \begin{align*}
        x\in S(C,M)&{}\iff P(\{x_{v_1},\dots,x_{v_n}\},M)=true
                   ...
                   &{}\iff \forall_{1\leq k\leq n}\colon P_k(\{x_{v_1},\dots,x_{v_k}\},M\}=true
    \end{align*}
    ...
    The following implication clearly still holds, with only the reverse no
    longer guaranteed:
    \begin{align*}
        x\in S(C,M)\implies\forall_{1\leq k\leq n}\colon\tilde P_k(\{x_{v_1},\dots,x_{v_k}\},M\}=true
    \end{align*}
