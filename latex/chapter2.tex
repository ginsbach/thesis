...
\subsection{Solving of SSA Constraint Problems}
    ...
    solutions $S(C,M)$ for some concrete $C$ and $M$.
    ...
    evaluation of $P$ can be performed efficiently.
    The solver therefore uses the structure of the boolean predicate and
    backtracking.

\begin{figure}[p]
    \begin{definition}{Partial Constraint Problem}{partconstr}
        Given a constraint problem $C=(V,P)$ and some order
        $V=\{v_1,\dots,v_n\}$ of the variables, the
        {\em $k$-th partial set of varibles} $V_k$ is defined as $V_0:=\{\}$ and
        \begin{align*}
            V_k:={}&\{v_1,\dots,v_k\}\quad\text{ for }k=1,\dots,n.
        \end{align*}

        The {\em $k$-th restricted constraint predicate}
        $R_k:\mathbb N^{V_k}\times\mathcal M\rightarrow\{true, false\}$ is
        defined by
        \begin{align*}
            R_k(\{x_{v_1},\dots,x_{v_k}\},M):={}&\left\{\begin{array}{l}
                    true\quad \text{if }P(\{x_{v_1},\dots,x_{v_n}\},M)=true\text{ for some }x_{v_{k+1}},\dots,x_{v_n}\\
                    false\quad \text{otherwise}.
                \end{array}\right.
        \end{align*}

        Finally, the {\em $k$-th partial constraint problem} $C_k=(V_k,P_k)$ is
        defined by the predicate $P_k$ with $P_0:=R_0$ and
        \begin{align*}
            P_k(\{x_{v_1},\dots,x_{v_k}\},M):={}&\left\{\begin{array}{l}
                                true\quad \text{if }R_k(\{x_{v_1},\dots,x_{v_k}\},M)=true\\
                       \phantom{true\quad \text{}}\mathrel{\lor}R_{k-1}(\{x_{v_1},\dots,x_{v_{k-1}}\},M)=false\\
                                false\quad \text{otherwise}.
                            \end{array}\right.
        \end{align*}
    \end{definition}
...
\end{figure}
\begin{figure}[p]
    \begin{definition}{Approximate Partial Constraint Problem}{approxconstr}
        Given a constraint problem $C=(V,P)$ and some order
        $V=\{v_1,\dots,v_n\}$ of the variables, 
        $\tilde C_k=(V_k,\tilde P_k)$ are called
        {\em approximate partial constraint problem}s if the predicates fulfill
        the following property
        \begin{align*}
            [P_k(x,M)=true&{}\implies\tilde P_k(x,M)=true]\text{ for all }k=1,\dots,n\\
            [P_k(x,M)=false&{}\implies\exists k\leq k'\leq n\colon P_{k'}(x,M)=false]\text{ for all }k<1,\dots,n
        \end{align*}
    \end{definition}
    \vspace{-0.25em}
\end{figure}
\begin{algorithm}[p]
    ...
    \begin{algorithmic}[1]
        \Procedure{Detect}{$\mathcal F$, $k\gets 0$, $x\gets\emptyset$}\vspace{-0.33em}
            \State $k\gets1$,\quad$x_{v_k}\gets1$\vspace{-0.33em}
            \While{$true$}\vspace{-0.33em}
                \State $x_{v_k}\gets min\{y\in\mathbb N\mid y\geq x_{v_k}\mathrel\land\tilde P_k(\{x_{v_1},\dots,x_{v_{k-1}},y\},M)=true\}$\vspace{-0.33em}
...
\end{algorithm}
    ...
    In order to formulate the backtracking algorithm, the concept of
    {\em partial constraint problems} from \autoref{def:partconstr} is used.
    This allows the constraint problems to be decomposed into multiple
    constraint problems on partial solutions, as demonstrated in the following
    equation:

    \begin{align*}
        x\in S(C,M)&{}\iff P(\{x_{v_1},\dots,x_{v_n}\},M)=true
                      \iff R_n(\{x_{v_1},\dots,x_{v_n}\},M)=true\\
                   &{}\iff P_n(\{x_{v_1},\dots,x_{v_n}\},M)=true\mathrel{\land}
                           R_{n-1}(\{x_{v_1},\dots,x_{v_{n-1}}\},M)=true\\
                   &{}\iff \forall{1\leq k\leq n}\colon P_k(\{x_{v_1},\dots,x_{v_k}\},M\}=true
    \end{align*}

    This directly suggests an algorithm for solving SSA constraint problems by
    incrementally solving $C_0$, then $C_1$, etc.
    However, even when the original predicate $P$ was straighforward to verify,
    it is not clear that the same would hold for the partial constraint
    problems.
    \autoref{def:approxconstr} therefore introduces a generalisation of the
    concept with a weaker condition, the
    {\em approximate partial constraint problem}.
    The following implication clearly still holds for this property.
    \begin{align*}
        x\in S(C,M)\iff\forall{1\leq k\leq n}\colon\tilde P_k(\{x_{v_1},\dots,x_{v_k}\},M\}=true
    \end{align*}

    This gives the basic backtracking algorithm for implementing a solver in
    \autoref{backtrackalg}.
    The variable $k$ is initialised as $1$ in line 2 and increases with the size
    of the partial solution in the loop spanning lines 3--15.
    ...
    Otherwise, the solution is either complete, or the algorithm increases $k$
    in line $9$ and continues the search in the next iteration.
    
    In order to implement this algorithm, the set in line $4$ has to be
    available.
    This requires the study of the structure of SSA constraint programs.

\newpage
\subsection{Structure of Basic SSA Constraint Problems}

    This section defines the building blocks that are necessary to construct a
    large class of useful SSA constraint problems.
    It also derives construction rules for corresponding approximate
    partial constraint problems that allow the implementation of efficient
    solvers.
    The most basic construction rules are the element-of constraint
    problem in \autoref{def:elementofconstr}, the conjunction and disjunction
    constraint problems in \autoref{def:conjconstr} and the extension of
    constraint problems in \autoref{def:constrext}.

\begin{figure}[h]
    \begin{definition}{Element-of Constraint Problem}{elementofconstr}
        For a set of tuples $S\subset\mathbb N^V$, the element-of
        constraint problem $C^S:=(V,P)$ is given by
        \begin{align*}
            P(x,M)=true\iff x \in S.
        \end{align*}
    \end{definition}

    \begin{definition}{Conjunction and Disjunction Constraint}{conjconstr}
        For two constraint problems $C=(V,P)$ and $C'=(V,P')$, the
        conjunction constraint $C\mathrel\land C':=(V,P\mathrel\land P')$ is
        given by
        \begin{align*}
            P\mathrel\land P'(x,M):=P(x, M)\mathrel\land P'(x,M).
        \end{align*}

        Accordingly, the conjunction constraint
        $C\mathrel\lor C':=(V,\mathrel\lor )$ is given by
        \begin{align*}
            P\mathrel\lor P'(x,M):=P(x, M)\mathrel\lor P'(x,M).
        \end{align*}
    \end{definition}

    \begin{definition}{Extension of Constraint Problems}{constrext}
        For an SSA constraint problem $C=(V,P)$ and an injection
        $i:V\hookrightarrow W$, the extension $C\circ p$ of the constraint problem
        is defined as $C\circ i:=(W,P\circ (i\times id))$.
    \end{definition}
\end{figure}

    Corresponding tho these three classes of SSA constraint problems, there
    are rules to form approximate partial constraint problems.

\begin{figure}[h]
    \begin{tabular}{|c|c|}
        \hline
        original & approximate partial constraint problem\\
        \hline\hline
        $C^S$               & $C_k^S$\\
        $C\mathrel\land C'$ & $\tilde C\mathrel\land \tilde C'$\\
        $C\mathrel\lor C'$  & $\tilde C\mathrel\lor \tilde C'$\\
        $C\circ p$          & true for $x\notin V$ otherwise P(x)\\
        \hline
    \end{tabular}
\end{figure}
