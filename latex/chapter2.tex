...
\subsection{Solving of SSA Constraint Problems}
    ...
    The objective of solvers for SSA constraint problems is to efficiently
    compute $S_M(V,C)$ for some concrete $(V,C)$ and $M$.
    This is fundamentally a search problem:
    The space of $\mathbb N^V$ has to be searched for values satisfying $C$.
    $\mathbb N^V$ is countable but infinite, so this requires non-trivial
    computational approaches, even when the direct evaluation of $C$ for any
    potential solution can be performed efficiently.
    The solver therefore uses the structure of the boolean predicate to find
    partial solutions in smaller search spaces, which are incrementally
    extended.

\begin{figure}[p]
    \begin{definition}{Partial Constraint Problem}{partconstr}
        Given a constraint problem $(V,C)$ and some order
        $V=\{v_1,\dots,v_n\}$ of the variables, the
        {\em $k$-th partial set of varibles} $V_k$ is defined as $V_0:=\{\}$
        and
        \begin{align*}
            V_k:={}&\{v_1,\dots,v_k\}\quad\text{ for }k=1,\dots,n.
        \end{align*}

        The {\em $k$-th partial constraint predicate}
        $R_k[C]:\mathbb N^{V_k}\times\mathcal M\rightarrow\{1,0\}$ is defined by
        \begin{align*}
            R_k[C](\{x_{v_1},\dots,x_{v_k}\},M):={}&\left\{\begin{array}{l}
                    1\quad \text{if }C(\{x_{v_1},\dots,x_{v_n}\},M)=1\text{ for some }x_{v_{k+1}},\dots,x_{v_n}\\
...
\end{figure}
\begin{figure}[p]
    \begin{definition}{Partition of Constraint Problem}{approxconstr}
        Given an SSA constraint problem $(V,C)$ and partial sets of variables
        $V_k$, a collection of predicates $(C_k)_{k=1\dots n}$ is a
        {\em partition} of $C$ if
        \begin{align*}
            [R_k[C](x,M)=1&{}\implies C_k(x,M)=1]\text{ for all }k=1,\dots,n\\
            [R_k[C](x,M)=0&{}\implies\exists k'\colon C_{k'}(x,M)=0]\text{ for all }k<1,\dots,n
        \end{align*}
    \end{definition}
    \vspace{-0.25em}
\end{figure}
\begin{algorithm}[p]
    ...
    \begin{algorithmic}[1]
        \Procedure{Detect}{$M,(v_k)_{k=1\dots n},(C_k)_{k=1\dots n}$}\vspace{-0.33em}
            \State $k\gets1$,\quad$x_{v_k}\gets1$\vspace{-0.33em}
            \While{$true$}\vspace{-0.33em}
                \State $x_{v_k}\gets min\{y\in\mathbb N\mid y\geq x_{v_k}\mathrel\land C_k(\{x_{v_1},\dots,x_{v_{k-1}},y\},M)=1\}$\vspace{-0.33em}
                \If{$x_{v_k}<\infty$}\vspace{-0.33em}
                    \If{$k=n$}\vspace{-0.33em}
                        \State {\bf yield} $x$,\quad$x_{v_k}\gets x_{v_k}+1$\vspace{-0.33em}
                    \Else\vspace{-0.33em}
                        \State $k\gets k+1$,\quad$x_{v_k}\gets1$\vspace{-0.33em}
                    \EndIf
                \Else
                    \State $k\gets k-1$\vspace{-0.33em}
                    \If{$k\geq1$}\vspace{-0.33em}
                        \State$x_{v_k}\gets x_{v_k}+1$\vspace{-0.33em}
                    \Else\vspace{-0.33em}
                        \State {\bf exit}
                    \EndIf
                \EndIf
...
\end{algorithm}
    ...
    \autoref{def:partconstr} introduces notation for SSA constraint problems,
    which is used to define partitions of SSA constraint problems in
    \autoref{def:approxconstr}.
    This definition is the foundation that allows backtracking on SSA constraint
    problems, due to the following equivalence:
    \begin{align*}
        x\in S_M(V,C)\implies{}&R_k[C](\{x_{v_1},\dots,x_{v_k}\},M)=1\text{ for all }k\\
                     \implies{}&C_k(\{x_{v_1},\dots,x_{v_k}\},M)=1\text{ for all }k\\[1em]
        x\notin S_M(V,C)\implies{}&R_n[C](x,M)=0\\
                        \implies{}&C_k(\{x_{v_1},\dots,x_{v_k}\},M)=0\text{ for some }k
    \end{align*}

    Therefore,
    $(x\in S_M(V,C)\iff C_k(\{x_{v_1},\dots,x_{v_k}\},M)=1\text{ for all }k)$
    holds for any partition $(V_k,C'_k)_{k=1\dots n}$ of an SSA constraint
    problem $(V,C)$.
    This means that the solutions to the original constraint problem can be
    identified by incrementally solving
    $(V_1,C'_1),(V_2,C'_2),\dots,(V_n,C'_n)$, extending the partial solutions
    one dimension at a time.

    This justifies the backtracking approach in \autoref{backtrackalg}.
    The variable $x$ iterates over $\mathbb N^V$ in order to find
    solutions, while the variable $k$ tracks the dimensions of this partial
    solution that have been verified so far.
    Both are initialised at the beginning of the algorithm in line 2 and are
    then manipulated during the execution of the loop spanning lines 3--15.
    In each iteration, the algorithm tries to assign a valid value to the latest
    element in the partial solution in line 4.
    If this fails, the algorithm backtracks in lines 10--15.
    Otherwise, the solution is either complete (lines 6--7), or the algorithm
    increases the size of the partial solution in line $9$ and continues the
    ...
    It is computed from the parition of the SSA constraint program that is given
    to the algorithm as a parameter.
    The next section derives a large class of useful SSA constraint problems,
    for which this set can be efficiently computed.
...
\subsection{Structure of Basic SSA Constraint Problems}
    ...
    in their own, as previously seen.
    Implicitly, this often requires the definition of constraint problems on a
    ...
    partitions can be easily constructed as in \autoref{def:canonpart}.
    For element-of constraints, the actual partial constraint problems are used.
    This is viable, because the set $S$ is explicitly given with the asumption
    that it is relatively small.
    The canonical partition for conjunctions is given my conjunction of the
    individual canonical partitions.
    It is not obvious that this satisfies the properties in
    \autoref{def:approxconstr}, but consider the following:
    \begin{align*}
        P_k[C]\mathrel\land\hat R_k[C'](x,M)=0


        P_k[C\mathrel\land C']&(x,M)=1\\
                        \iff{}&R_k[C\mathref\land C'](\{x_{v_1},\dots,x_{v_k}\},M)=1\\
                \mathrel\lor{}&R_{k-1}[C\mathref\land C'](\{x_{v_1},\dots,x_{v_{k-1}}\},M)=0\\
                    \implies{}&(R_k[C](\{x_{v_1},\dots,x_{v_k}\},M)=1\mathrel\land R_k[C'](\{x_{v_1},\dots,x_{v_k}\},M)=1)\\
                \mathrel\lor{}&R_{k-1}[C\mathref\land C'](\{x_{v_1},\dots,x_{v_{k-1}}\},M)=0\\
        P_k[C]\mathrel\land\hat P_k[C'](x,M)=1
    \end{align*}
    

\begin{figure}[p]
    \begin{definition}{Element-of Constraint Problem}{elementofconstr}
        For a set of tuples $S\subset\mathbb N^V$, the element-of
        constraint problem $(V,E_S)$ is given by
        \begin{align*}
            E_S(x,M)=\left\{\begin{array}{l}
                                1\text{\quad if }x \in S\\
    ...
    \begin{definition}{Conjunction and Disjunction Constraint}{conjconstr}
        ...
        \begin{align*}
            &C\mathrel\land C'(x,M):=C(x, M)\mathrel\land C'(x,M)\\
            &C\mathrel\lor  C'(x,M):=C(x, M)\mathrel\lor C'(x,M).\\
        \end{align*}
    \end{definition}
    ...
    \begin{definition}{Extension of Constraint Problems}{constrext}
        ...
        \begin{align*}
            C^W(x,M):=C(\{x_{i(v)}\}_{v\in V},M).
        \end{align*}
    \end{definition}

    \begin{definition}{Canonical Partition}{canonpart}
        For constraint problems that are constructed according to the previous
        rules, the following {\em canonical partitions} are  defined
        \begin{align*}
            P_k[E_S]:={}&R_k[E_S]\\
            P_k[C\mathrel\land C']:={}&P_k[C]\mathrel\land P_k[C']\\
            P_k[C\mathrel\lor C'] :={}&(P_1[C]^V\lathrel\land\dots\mathrel\land P_k[C])\mathrel\lor(P_1[C']^V\lathrel\land\dots\mathrel\land P_k[C'])\\
            P_k[C^W]:={}&\left\{\begin{array}{ll}
                                         P_k[C]^W&\text{ if }v_k\in W\\
...
\begin{figure}[p]
    \begin{blackwhitebox}{Original SSA constraint problem}
        ...
        \begin{align*}
            ...
            P(v,M)&{}=\left(         (v_\text{step},1)
                                     \in C_\mathcal{F}
                      \mathrel{\land}(v_\text{phi},v_\text{update})
                                     \in DFG_\mathcal{F}^*\right.\\
                     &\mathrel{\land}(v_\text{step},v_\text{update})
                                     \in DFG_\mathcal{F}^*
                      \mathrel{\land}(v_\text{phi}, phi)
                                     \in I_\mathcal{F}\\
               &\left.\mathrel{\land}(v_\text{update}, add)
                                     \in I_\mathcal{F}
                      \mathrel{\land}(v_\text{update},v_\text{phi})
                                     \in DFG_\mathcal{F}^*\right)
        \end{align*}
    \end{blackwhitebox}
    \begin{blackwhitebox}{Approximate Partial Constraint problems}
        \setlength{\abovedisplayskip}{0pt}
        \setlength{\belowdisplayskip}{0pt}
        \vspace{-0.5em}
        \begin{align*}
            V_1={}&\{\text{phi}\}\\
            \tilde P_1(v,M)=({}&v_\text{phi}
                               \in\{x_1\mid x\in DFG_\mathcal{F}\}\\
                \mathrel\land{}&v_\text{phi}
                               \in\{x_1\mid x\in I_\mathcal{F}, x_2=phi\}\\
                \mathrel\land{}&v_\text{phi}
                               \in\{x_2\mid x\in DFG_\mathcal{F}\})\\[1em]
            V_2={}&\{\text{phi}, \text{update}\}\\
            \tilde P_2(v,M)=({}&v_\text{update}
                               \in\{x_2\mid x\in DFG_\mathcal{F},x_1=v_{phi}\}\\
                \mathrel\land{}&v_\text{update}
                               \in\{x_2\mid x\in DFG_\mathcal{F}\}\\
                \mathrel\land{}&v_\text{update}
                               \in\{x_1\mid x\in I_\mathcal{F},x_2=add\}\\
                \mathrel\land{}&v_\text{update}
                               \in\{x_2\mid x\in DFG_\mathcal{F},x_2=v_{phi}\})\\[1em]
            V_3={}&\{\text{phi}, \text{update}, \text{step}\}\\
            \tilde P_3(v,M)=({}&v_\text{step}
                               \in\{x_1\mid x\in C_\mathcal{F}, x_2=1\}\\
                \mathrel\land{}&v_\text{step}
                               \in\{x_1\mid x\in DFG_\mathcal{F}, x_2=v_{update}\})
        \end{align*}
    \end{blackwhitebox}
    \vspace{10cm}
\end{figure}
