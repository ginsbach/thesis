...
\subsection{Solving of SSA Constraint Problems}
    ...
    solutions $S_M(V,C)$ for some concrete $(V,C)$ and $M$.
    This is fundamentally a search problem: the space of $\mathbb N^V$ has to be
    searched for values satisfying $C$.
    ...
    evaluation of $C$ can be performed efficiently.
...
\begin{figure}[p]
    \begin{definition}{Partial Constraint Problem}{partconstr}
        ...
        Finally, the {\em $k$-th partial constraint problem} $(V_k,P_k[C])$ is
        defined using the {\em $k$-th partial constraint predicate}
        $P_k[C]:\mathbb N^{V_k}\times\mathcal M\rightarrow\{1,0\}$, given by
        $P_1[C]:=R_1[C]$ and
        \begin{align*}
            P_k[C](\{x_{v_1},\dots,x_{v_k}\},M):={}&\left\{\begin{array}{l}
                                1\quad \text{if }R_k(\{x_{v_1},\dots,x_{v_k}\},M)=1\\
                       \phantom{1\quad \text{}}\mathrel{\lor}R_{k-1}(\{x_{v_1},\dots,x_{v_{k-1}}\},M)=0\\
                                0\quad \text{otherwise}.
                            \end{array}\right.
        \end{align*}
    \end{definition}
...
\begin{figure}[p]
    \begin{definition}{Partition of Constraint Problem}{approxconstr}
        ...
        $V_k$, a collection of predicates $(C'_k)_{k=1\dots n}$ is a
        {\em partition} of $C$ if
        \begin{align*}
            [P_k[C](x,M)=1&{}\implies C'_k(x,M)=1]\text{ for all }k=1,\dots,n\\
            [P_k[C](x,M)=0&{}\implies\exists k\leq k'\leq n\colon C'_{k'}(x,M)=0]\text{ for all }k<1,\dots,n
        \end{align*}
    \end{definition}
    \vspace{-0.25em}
\end{figure}
    ...
    \begin{algorithmic}[1]
        \Procedure{Detect}{$M,(v_k)_{k=1\dots n},(C_k)_{k=1\dots n}$}\vspace{-0.33em}
            ...
            \While{$true$}\vspace{-0.33em}
                \State $x_{v_k}\gets min\{y\in\mathbb N\mid y\geq x_{v_k}\mathrel\land C_k(\{x_{v_1},\dots,x_{v_{k-1}},y\},M)=true\}$\vspace{-0.33em}
...
\end{algorithm}
    ...
    In order to formulate the backtracking algorithm, any
    {\em constraint problem partition} as in \autoref{def:partconstr} can be
    used.
    ...
    \begin{align*}
        x\in S_M(V,C)\implies{}&R_k[C](x,M)=1\text{ for all }k
                     \implies P_k[C](x,M)=1\text{ for all }k\\
                     \implies{}&C'_k(x,M)=1\text{ for all }k\\[1em]
        x\notin S_M(V,C)\implies{}&R_n[C](x,M)=0\\
                        \implies{}&R_1[C](x,M)=0\text{ or }\exists k_{min}>1\text{ minimal with }R_{k_{min}}[C](x,M)=0\\
                        \implies{}&P_1[C](x,M)=0\text{ or }\exists k>1:P_k[C](x,M)=0\\
                        \implies{}&C'_k(x,M)=0\text{ for some }k
    \end{align*}

    Therefore, $x\in S_M(V,C)\iff C'_k(x,M)=1\text{ for all }k$.
    This justifies the basic backtracking algorithm for implementing a solver in
