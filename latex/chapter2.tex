...
\section{Deriving an SSA Model}
    ...
    generally to derive a model for the execution of such programs.
    Instead, the section investigates the static structure, focusing on
    precise notation for the commonalities of existing SSA intermediate
    representations.
    This requires naming conventions for some of the basic features of
    SSA programs.
    The remainder of this section adheres to \Cref{def:ssa} when referring to
    these features.
...
\label{sec:remainingstructure}
    ...
    structures, individual instructions are encoded by their opcodes.
    ...
    Babylonian method, as previously introduced in \Cref{ssaexample}.
    The function has 11 instructions separated into four basic blocks, with the
    ...
    In order to demonstrate the equivalence in \Cref{fig:separation},
...
\begin{enumerate}
    ...
    \end{itemize}
    \item Basic block labels and register names can be chosen arbitrarily, they
          carry no meaning.
    \item The arguments of all operations can be immediately filled in from the 
...
\end{figure}
    ...
    the per-value information at the bottom left of \Cref{fig:separation},
...
\end{figure}
    ...
    The model in \Cref{def:ssamodel} has several advantages over such an
    approach.
...
\subsection{Graph Projections}
    ...
    Additional notation is introduced in \Cref{def:convenience} for
    convenience.
