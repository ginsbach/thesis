...
\end{figure}
    ...
    state that is shared by several of the \texttt{BacktrackingPart}
    objects for the change in size of the partial solution.
    ...
    \begin{itemize}
    \item For element-of constraint problems, $R_k(M,x)$ can be implemented as a
          sorted list of type
          \begin{lstlisting}[language=C++]
...
template<int k> using R_<k+1> = vector<pair<int,R_<k>>
          \end{lstlisting}
          The \texttt{BacktrackingPart} object contains an iterator that is
          initialised via \texttt{begin} and then incremented in
          \texttt{skip\_invalid}.
          Finally, \texttt{resume} does nothing, and \texttt{fixate} simply
          dereferences the iterator that is already in position.
    ...
    \item For disjunction constraints, $R_k(M,x)$ is simply a pair of booleans.
          The \texttt{skip\_invalid} member calls thoe underlying
          \texttt{BacktrackingPart} object for which $R_k$ is not $0$ and
          chooses the smallest value.
          The \texttt{begin}, \texttt{fixate} and \texttt{resume} member
          function update $R_K$ and call underlying objects when appropriate.
    \end{itemize}
