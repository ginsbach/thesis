...
double dot(double* a, double *b, uint64_t n)
{
  double d = 0.0;
  for(int i = 0; i < n; i++)
    d += a[i]*b[i];
...
\paragraph*{b)} LLVM IR representation of the {\tt dot} product function
\begin{lstlisting}[language=C]
double dot(double* a, double *b, uint_64_t n)
{
  double d = 0.0;
  for(int i = 0; i < n; i++)
    d += a[i]*b[i];
...
\end{lstlisting}
\vspace{10cm}
\caption{Features of LLVM IR explained with an example function.}
\label{llvmirexample}
\end{figure}
