...
void solver(vector<BacktrackingPart*> P) {
    ...
    \end{lstlisting}
    \caption{Backtracking Solver as implemented in C++.}
    \label{cppsolver}
\end{figure}
    ...
    follows.
    \begin{itemize}
    \item For element-of constraint problems, $R_k(M,x)$ is implemented as a
          sorted array of type
          \begin{lstlisting}[language=C++]
template<unsigned k> class R_;
template<>      using R_<1>   = vector<int>;
template<int k> using R_<k+1> = vector<pair<int,R_<k>>
          \end{lstlisting}
          The \texttt{BacktrackingPart} object contains an iterator into this
          array that is initialised via \texttt{begin} and then incremented in
          \texttt{skip\_invalid} until either hitting the end or
          finding a value $\geq$ to the function argument \texttt{c}.
          Finally, \texttt{resume} does nothing, and \texttt{fixate} simply
          dereferences the iterator that is already in position from the
          previous \texttt{skip\_invalid} call.
    \item For conjunction constraint problems, \texttt{begin}, \texttt{fixate}
          ...
          either of them fail.
    \item For disjunction constraints, $R_k(M,x)$ is simply a pair of booleans.
          Calls to \texttt{skip\_invalid} assed passed on to those underlying
          \texttt{BacktrackingPart} objects for which $R_k$ is $true$.
          The smallest resulting value is chosen if any succeeded.
          The value of $R_k$ is updated in \texttt{fixate} by checking
          \texttt{skip\_invalid} of the underlying objects.
