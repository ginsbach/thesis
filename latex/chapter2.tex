...
\end{enumerate}
    ...
    Therefore, these structures remain opaque at this point and are modelled as
    arbitrary sets as in \Cref{def:isatypes}.

\begin{figure}[h]
\begin{definition}{Instruction Set and Type System}{isatypes}
    For an SSA compiler intermediate representation $L$, the
    {\it set of opcodes} is $Opcodes_L$ and the {\it set of types} is $Types_L$.
\end{definition}
\end{figure}
...
\end{figure}
    ...
    They mark all the values that are function parameters or globals without
    attaching any extra information.
    This is sufficient for parameters because their names carry no significance
    and their positionality distinguishes them.
    However, from a program-wide perspective, global variable names are
    significant.
    The SSA model operates per function, but global names can in practice still
    be convenient in some cases.
    This is particularly true for the names of standard library functions.
    When required, these can be encoded in the type model, as in
    \Cref{chapter:candl}.

\subsection{Graph Projections}

    The individual components in the SSA model are each expressed as a set of
    tuples.
    In order to conveniently manipulate these structures, 
    \Cref{def:convenience} introduces some notation.
    ...
    Correspondingly, $tails$ removes the first element of all tuples within a
    set.
    For example, $tails(I_\mathcal F)$ identifies the indices of all
    ...
    resulting in a ordinary graph structures.

\subsection{The LLVM Compiler Framework}

    The previously introduced SSA model is generic applies to all SSA compiler
    intermediate representations.
    However, the model needs to be specialised to a specific compiler
    intermediate representations in order to use it on real compiler problems.
...
\label{sec:constraintprogramming}
    ...
    For this purpose, \Cref{def:modelrepresentations} is introduced first,
