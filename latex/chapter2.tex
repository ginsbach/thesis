    ...
    This chapter introduces the underlying methodology of this approach.
    Single static assignment (SSA) form is formalised into suitable model as the
    basis for constraint programming.
    With this model, constraint problems that impose restrictions on fragments
    of SSA code are derived.
    Detecting adhering code sections in SSA programs is then equivalent to
    solving the constraint problem.
    Finally, the structure of constraint problems is analysed and efficient
    solving techniques are derived.
    \vfill
...
\section{Introduction}
    ...
    \vfill
    {\bf Mid end representations} are designed to be suitable for the analysis
...
%    the eponymous SSA property being only one among them.
    ...
    Despite many differences, they share fundamental paradigms and many of the
    differences can be abstracted away as implementation specific details for
    this work.

    SSA form was developed as an improvement over previously used compiler
    intermediate representations.
    The eponymous static single assignment property applies to more general
    {\em linear intermediate representations} as an additional, as described in
    \citet{Torczon:2007:EC:1526330}.
...
\end{figure}
    ...
    they are based on a single sequence of instructions per function.
    These instructions operate on an abstract machine, which provides an
    unlimited number of identical registers and a well defined instruction set.
    The control flow is expressed as branch instructions, which redirects the
    execution conditionally or unconditionally.
    Consecutive instructions without branches are grouped as basic blocks.
    Within these basic blocks, instruction are executed in order of appearance.
    These basic blocks may have labels or be identified simply by enumerating
    them.
    Instruction arguments can be registers, constants or globals.
...
\begin{figure}[h]
    \begin{definition}{Backtracking Solution of Constraint Problems}{backtracking}
        ...
        \begin{align}
            C(M,x)=1\iff P_k(M,(x_{v_1},\dots,x_{v_{k-1}}),x_{v_k})=1\text{ for all }1\leq k\leq n.
        \end{align}
    \end{definition}
\end{figure}
