...
\end{figure}
    ...
    With \autoref{def:backtracking}, the backtracking search in
    \autoref{backtrackalg} can be implemented.
    The variable $x$ iterates over $\mathbb N^n$ in order to find
    ...
    then manipulated during the execution of the loop spanning lines 3--17.
...
\begin{figure}[h]
    \begin{definition}{Element-of Constraint Problem}{elementofconstr}
        ...
        dependent on the SSA model $M\in\mathcal M$ (i.e.\ $DFG_\mathcal F^*$),
    ...
    \begin{definition}{Conjunction and Disjunction Constraint}{conjconstr}
        ...
        \begin{align*}
            &C\mathrel\land C'(M,x):=C(M,x)\mathrel\land C'(M,x)\\
            &C\mathrel\lor  C'(M,x):=C(M,x)\mathrel\lor C'(M,x).
        \end{align*}
    \end{definition}
...
\end{figure}
    ...
    backtracking solutions can be directly constructed.
    For all the rules aside from the disjunction, this is straightforward as
    shown in \autoref{theo:theo1}, \autoref{theo:theo2} and
    \autoref{theo:theo3}.
    For the disjunction, the situation is only slightly more complicated, as
    demonstrated in \autoref{theo:theo4}.

    With these backtracking solutions available, an example can easily be
    computed.
...
\begin{figure}[p]
    \begin{theorem}{Backtracking Solution for Element-of Constraint Problems}{theo1}
        ...
        \begin{align*}
            P_k[E_S](M,x,y):=\left\{
                \begin{array}{ll}
                    1&\text{if }y\in\{s_{v_k}\mid s\in S(M),s_{v_1}=x_1,\dots,s_{v_{k-1}}=x_{k-1}\}\\
                    0&\text{otherwise}.
                \end{array}\right.
        \end{align*}
        give a backtracking solution of $(V,E_S)$.
        \tcblower
        \paragraph*{Proof:} The equivalence from \autoref{def:backtracking}
                            holds in both directions:
        \begin{align*}
            C(M,x)=1\iff{}& x\in S(M)\implies P_k[E_S](M,(x_{v_1},\dots,x_{v_{k-1}}),x_{v_k})=1\text{ for all }k\\
                \implies{}& P_n[E_S](M,(x_{v_1},\dots,x_{v_{n-1}}),x_{v_n})=1\\
                \implies{}& x\in S(M)\iff C(M,x)=1.
        \end{align*}
    \end{theorem}
    \begin{theorem}{Backtracking Solution for Conjunction Constraint problems}{theo2}
        ...
        \begin{align*}
            P_k[C\mathrel\land C'](M,x,y):=P_k(M,x,y)\mathrel\land P'_k(M,x,y)
        \end{align*}
        ...
        \paragraph*{Proof:} The equivalence from \autoref{def:backtracking}
                            holds in both directions:
        \begin{align*}
            C(M,x)=1\mathrel\land C'(M,x)=1\iff{}& P_k(M,(x_{v_1},\dots,x_{v_{k-1}}),x_{v_k})\text{ for all }k\\
                                  \mathrel\land{}& P'_k(M,(x_{v_1},\dots,x_{v_{k-1}}),x_{v_k})\text{ for all }k\\
                                           \iff P_k[C\mathrel\land C']&(M,(x_{v_1},\dots,x_{v_{k-1}}),x_{v_k})\text{ for all }k
        \end{align*}
    \end{theorem}
\end{figure}
...
\begin{figure}[p]
    \begin{theorem}{Backtracking Solution for Extensions of Constraint Problems}{theo3}
        ...
        solution $(P_k)_{k=1\dots n}$, a set $W$ with an injection
        $i:V\hookrightarrow W$ and an order $W=\{w_1,\dots,w_n\}$ that is
        compatible with the order on $V$ such that $v_k=w_{t(k)}$ with
        $t:\mathbb N\rightarrow\mathbb N$ strictly increasing, the predicates
        $(P_k[C^W])_{k=1\dots |W|}$ defined by
        \begin{align*}
            P_k[C^W](M,x,y):=\left\{
                \begin{array}{ll}
    ...
    \begin{theorem}{Backtracking Solution for Disjunction Constraint problems}{theo4}
        ...
        \begin{align*}
            P_k[C\mathrel\lor C'](M,x,y):={}&\left(P_l(M,(x_1,\dots,x_{l-1}),x_l)\text{ for }l=1\dots k-1\mathrel\land P_k(M,x,y)\right)\\
                              \mathrel\lor{}&\left(P'_l(M,(x_1,\dots,x_{l-1}),x_l)\text{ for }l=1\dots k-1\mathrel\land P'_k(M,x,y)\right)\\
        \end{align*}
        give a backtracking solution of $(V,C\mathrel\lor C')$.
        \tcblower
        \paragraph*{Proof:} The equivalence from \autoref{def:backtracking}
                            holds by assumption for the backtracking solutions
                            of $(V,C)$ and $(V,C')$, therefore
        \begin{align*}
            C(M,x)=1\mathrel\lor C'(M,x)=1\iff{}& P_k(M,(x_{v_1},\dots,x_{v_{k-1}}),x_{v_k})=1\text{ for all }k\\
                                  \mathrel\lor{}& P'_k(M,(x_{v_1},\dots,x_{v_{k-1}}),x_{v_k})=1\text{ for all }k.
        \end{align*}
        With the definition of $P_k[C\mathrel\lor C']$, this gives
        \begin{align*}
            C\mathrel\lor C'(M,x)=1\implies P_k[C\mathrel\lor C']&(M,(x_{v_1},\dots,x_{v_{k-1}}),x_{v_k})=1\text{ for all }k=1,\dots,n.
        \end{align*}
        The other direction is clear due to
        \begin{align*}
            P_n[C\mathrel\lor C']&(M,(x_{v_1},\dots,x_{v_{n-1}}),x_{v_n})=1\implies C\mathrel\lor C'(M,x)=1.
        \end{align*}
        The equivalence from \autoref{def:backtracking} therefore holds in both
        directions.
    \end{theorem}
\end{figure}
