...
\label{subsec:impl}
    ...
    identify suitable data structures efficiently implementing the backtracking
    algorithm in real code.
    This first requires rephrasing the backtracking algorithm first
    introduced in \Cref{backtrackalg}.
    \Cref{cppsolver} shows how this backtracking solver is implemented as a C++
    function.
    Most of the \texttt{solver} function in lines 12--43 maps directly onto
    \Cref{backtrackalg}, and needs no further explanation.
    However, there are some crucial details that need elaboration, mostly
    relating to the iteration over the backtracking solution in line 4 of
    \Cref{backtrackalg}.
