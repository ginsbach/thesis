...
\subsubsection{Data Structures for Constraint Classes}
    ...
    as shown with the definition of \texttt{Tree} on the first line of the top
    section of \Cref{partimplementations}.
...
\subsubsection{Computational Complexity}
    ...
    In practice, the nature of SSA constraint problems means that the same
...
\subsection{Additional SSA Constraint Problems}
    ...
    This is convenient only in disjunctions, for solution components that may be
    omitted.
    ...
    The value $ R_{val}+1$ is chosen as it is the smallest integer that can
    not be used as an index into the list of used values.

    {\it Generalised graph domination constraint problems} as in
    \Cref{def:domconstr} are versatile tools for analysing SSA models.
    ...
    \Cref{chapter:reductions} uses for defining kernel functions with
    ...
    constraint problems, the enumeration of $W$ must satisfy the property
    $w_a\in U,w_b\notin U\implies a<b$.
    For values in $U$, the backtracking solution simply passes through to the
    underlying constraint.
    Whenever a partial solution is determined over all values in $U$, an
    invokation to the full solver is performed that identifies all possible
    partial solutions over $V\setminus U$.
    The remaining steps of the backtracking solution then merely enforce this
...
\begin{figure}[p]
    ...
    \begin{definition}{Collect-all Constraint Problem}{collectconstr}
        ...
        each value in $V\setminus U$ is defined as
        \begin{align*}
            V^{\left<U,n\right>}=U\cup(\{1,\dots,n\}\times (V\setminus U)).
        \end{align*}

        For any $x\in\mathbb N^U$, the set of constraint solutions restricted to
        $x$ in $U$ is defined as
        \begin{align*}
            S_M(V,C)|_{x}=\{y\in S_M(V,C)\mid y_u=x_u\text{ for all }u\in U\}.
        \end{align*}
        ...
        \begin{align*}
            C^{\left<U,n\right>}(M,x)=1\iff{}&\left|S_M(V,C)|_{g(x)}\right|\leq n\\[-0.73mm]
                              ...
                              \mathrel\land{}&f_k(x)\equiv R_{ins}+1\text{ for all }k>\left|S_M(V,C)|_{g(x)}\right|\\[-0.73mm]
                              \mathrel\land{}&f_1(x),\dots,f_n(x)\text{ are in lexicographical order}.
        \end{align*}
    \end{definition}
\end{figure}
