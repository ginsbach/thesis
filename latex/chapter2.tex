...
\subsection{Application to LLVM}
    ...
    The name was originally an antony for ``Low Level Virtual Machine''.
    The project has been a huge success, front ends exist for a diverse set of
    langauges including C/C++, Haskell, Julia, Objective-C, Rust, Scala and
    CUDA.
...
\subsection{Constraint Program Example}
    ...
    We express this as an SSA constraint problem $C=(V,P)$ with the variables
    $V=\{\text{phi}, \text{update}, \text{step}\}$ and the predicate
...
\end{align*}
    ...
    This decomposes the predicate into a logical conjunction of a range of
    ...
    determined.
    Applied to the {\tt dot} function in \autoref{fig:derivemaths}, the set of
    constraint solutions contains a single element, corresponding to the loop
    iterator {\tt i}.
    This is demonstrated in \autoref{fig:constraintsolution}.
...
{\huge $\{\{\text{phi}:6,\text{update}:14,\text{step}:19\}\}$}

\caption{Example set of constraint solution for the loop detection predicate
         applied to \autoref{fig:derivemaths}.}
