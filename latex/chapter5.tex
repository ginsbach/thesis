    ...
    The Compiler Analysis Description Language (CAnDL) that was introduced in
    the previous chapter makes the constraint programming methods from
    \autoref{chapter:theory} directly available within the LLVM infrastructure.
    The previous chapter showed how this enables complex compiler analysis
    generation.
    This was demonstrated by reimplementing peephole optimisations and even a
    full polyhedral analysis.

    This chapter demonstrates new analysis functionality in CAnDL that is
    used for automatic parallsisation of a wide class of programs.
    This goes beyond parallelisation opportunities that are already
    accessible via conventional analysis methods.
    Instead, a previously neglected class of parallelisation opportunities is
    identified.
    Complex reductions and histogram computations may contain indirect memory
    access.

    These computations are not conventionally thought of as a single class and
    can be overlooked easily.
    However, this chapter demonstrates that several important benchmark programs
    can be reinterpreted as complex reductions and histogram computations to
    allow for automatic parallelisation of programs that were previously largely
