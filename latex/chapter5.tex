    ...
    introduced in \Cref{chapter:theory} directly accessible to the Clang
    ...
    previously overlooked  {\em computational idiom} -- a class of kernels that
    constitute bottlenecks of important benchmark programs and share algorithmic
    structure that allows for targeted acceleration and parallelisation
    approaches.
    ...
    Such loops were not typically studied as a single class of calculations
    previously, but this chapter demonstrates that grouping them together
    allows for automatic detection and parallelisation using the same methods.

    After introducing CReHCs, the chapter presents the Idiom Description
    ...
    kernel functions.
    These are an integral component of CRoHCs, where they model the reduction
    operators, but they are also necessary for the formulation of other
    computational idioms, such as stencil codes.
    
    Grouping reductions and histograms together and formulating
    CReHCs in IDL makes the automatic detection in C/C++ program code possible.
    The evaluation section of the chapter shows results from benchmarking this
    using parallelisation routines that were implemented to complement the
    detection.
...
\section{Introduction}
    ...
    Reductions occur widely in numerical applications and techniques for
    parallelising them were established by
    ...
    matrix multiplication kernel as a dot product.
    ...
    control flow.
    This chapter presents the Idiom Description Language (IDL) as an extension
    ...
    The chapter focuses on deriving the detection of reductions and assumes
...
\section{Motivation}
    ...
    to calculate in parallel several partial sums into thread-private copies of
    \texttt{sum}, which are then added to form the final value.
...
\end{figure}
    ...
    exploit them.
    However, such scalar reductions rarely dominate execution time in a program,
    ...
    The parallelisation of simple reductions alone, therefore, often has only a
...
\end{figure}
    ...
    before merging together the partial results.
...
\subsection{Constraint based formulation}
    ...
    The motivation section described how reductions could be parallelised and
...
\end{figure}
    ...
    \noindent
    The syntax of this atomic constraint is quite discriptive, but some
...
\subsection{Encapsulating Kernel Functions in IDL}
    ...
    \Cref{IDLscalarPart} encapsulates kernel functions in an IDL constraint
    specification.
...
\paragraph*{Polly-Reduction}
    ...
    \citet{Doerfert2015Polly} develop a compiler analysis and transformation
    system that detects reductions within code captured by the polyhedral model.
...
\multirow{7}{*}{\bf Parboil}
 ...
 & mri-gridding & Magnetic Resonance Imaging - Gridding\\[-2.4mm]
...
\multirow{12}{*}{\bf Rodinia}
 & backprop       & Back Propagation\\[-2.4mm]
 & bfs            & Breadth-First Search\\[-2.4mm]
 ...
 & particlefilter & Object Location Estimator for Noisy Measurements\\[-2.4mm]
...
\end{figure}
    ...
    \paragraph*{Intel ICC}
    The Intel ICC compiler was more successful than Polly at recognising
    reductions:
    ...
    The Intel ICC compiler is more robust and does not require SCoPs as a
    precondition for its analysis.
    On the well structured NAS benchmarks, it performs well but fails to detect
    any reductions in IS.
    Surprisingly it also does not detect reductions in SP, while Polly does.
    ...
    On the less well structured Parboil benchmarks, ICC fails to detect many of
    ...
    Finally, ICC recognises many of the scalar reductions in Rodinia, but is
...
\subsection{Performance}
    ...
    The speedup that was achieve by exploiting reduction parallelism on
    histogram is detailed in figure \ref{speedup-figure}.
    ...
    are compared against the manually optimised parallel benchmark versions that
    ...
    parallelism and outperforms our reduction-based parallelisation.

    On the ``IS'' benchmark, the IDL-based parallelisation results in $2.9x$
    speedup, compared to $6.3x$ speedup of the manual parallel version.
    ...
    speedup of $35.7x$.
    The parallel version of this program is provided by the original
    implementers to benchmark agains is implemented poorly using a critical
    section.
...
\section{Conclusion}
    ...
    The chapter developed a new compiler-based approach to recognise a broad
    ...
    The approach proved robust when evaluated on C versions of three well-known
    benchmark suites: NAS, Parboil and Rodinia.
    It detected more reductions than existing approaches and was alone in being
    able to detect computationally intense histogram reductions.
    Such reductions were shown to give significant performance improvements.

    \paragraph*{Outlook}
    The chapter introduced a computational idiom in detail and provided the
    Idiom Detection Language (IDL) with the required tools to automatically
    recognise the idiom during compilation.
    \Cref{chapter:idioms} uses the methods 
 to recognise a range of other
    common algorithmic structures, together resulting in significant coverage
    of benchmark bottlenecks.
