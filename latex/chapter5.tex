    ...
    The previously disregarded Complex Reduction and Histogram Computations
    (CReHCs) are identified as a {\em computational idiom} -- a class of program
    kernels that constitute bottlenecks of important benchmark programs and that
    share common structure which allows for specific, targeted acceleration and
    ...
    previously, but this chapter demonstrates that they share structure that
    allows for automatic detection and parallelisation.
    ...
    The additional language features of IDL enable it to capture well-behaved
    kernel functions.
    These are an integral component of CRoHCs, where they model the reduction
    operators, but they also feature prominently in other idioms, such as
    stencil codes.
...
\section{Introduction}
    ...
    parallelising them were established, among others by
    \citet{rauchwerger1999lrpd,yu2006adaptive,Jradi2017fast}.
    ...
    The constraint programming approach derived in \Cref{chapter:theory},
    however, is not fundamentally inhibited by indirect memory accesses.
    Furthermore, the previous \Cref{chapter:candl} already used this to
    constructed the constraint programming language CAnDL that can capture
    ...
    During compilation, the CAnDL solver automatically identifies subsets within
    LLVM IR that constitute a reduction operation.
...
\subsection{Encapsulating Kernel Functions in IDL}
    ...
    graph dominance constraint, which is in lines 18--20.
    ...
    lines 2--6.
    Finally, lines 7--9 make sure that the reduction value actually leaves the
...
\end{figure}
    ...
    reductions:
    It detected 25 out of 38 reductions in NPB, 3 out of 11 in Parboil and
...
\section{Conclusion}
    ...
    This chapter developed a new compiler based approach that automatically
    detects a wide class of reductions.
