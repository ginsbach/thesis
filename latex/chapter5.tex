    ...
    In the previous chapter, the CAnDL constraint programming language was
    introduced, which can generate compiler analysis functionality from terse
    and easily scalable, declarative problem specifications.
    This chapter takes a broader view of what automatic compiler analysis can
    entail with such a powerful tool at hand.
    Being freed from having to implement complex optimisation passes manually,
    the chapter explores the complexity to which CAnDL can be driven, capturing
    broader computational concepts than what was demonstrated previously.

    The goal of this chapter is to take abstract algorithmic concepts that were
    previously explored outside the context of compiler analysis --
    computational idioms -- and to formalize them as CAnDL specifications,
    enabling automatic detetion in optimising compilers.
    The main source of inspiration will be the high performance and scientific
    computing community.

    Heterogeneous acceleration will serve as the motivation for this effort.
    As many scientific codes are often structured around idiomatic performance
    bottlenecks, parallelisation efforts on heterogeneous hardware that focus
    entirely on computational idioms can achieve significant runtime
    improvements.
    In cooperation with existing numeric libraries and domain specific code
    generators, this chapter will present an idiom centric heterogeneous
    auto-parallelising compiler.
    The focus is thus on calculations that are well supported by established
    APIs: sparse and dense linear algebra, stencil codes and generalized
    reductions and histograms.
    Idioms are then mapped to BLAS libraries, cuSPARSE and clSPARSE and two
    DSLs: Halide and Lift.

    Built on top of the CAnDL implementation in LLVM, the implemented
    funcionality is directly accessible in the widely used clang C/C++
    compilers.
...
\section{Introduction}
    ...
    often disappoints \cite{lee09openmp}, effectively requiring rewrites despite
    the functional portability.
    ...
    portability in mind \cite{clblas,cublas,cusparse,clsparse}.
    More narrow domain specific languages (DSLs) \cite{Franchetti09OL,
    Rompf:2012:LMS:2184319.2184345} have been proposed, attempting to deliver
    both portability and performance \cite{Ragan-Kelley2013Halide}.
    However this class is quickly evolving and most of them are academic
    projects with unclear long term support.
    ...
    The idioms are formulated using CAnDL from \autoref{chapter:candl}, which
    allows the concise formulation and fast detection in optimised LLVM IR code.
