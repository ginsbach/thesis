...
\multirow{4}{*}{\bf ifblock} & $t_9\leftarrow sqrt(-2.0\cdot log(t_7) / t_7)$\\[-2mm]
                             ...
                             & $\text{goto }uncond$\\
\hline
\multirow{2}{*}{\bf uncond} & $sx''\leftarrow\Phi(entry:sx,\ ifblock:sx')$\\[-2mm]
                            & $\text{return }sx''$\\
\hline
\end{tabular}
...
%    variabls from outer loops).
    ...
    This can be encapsulated as shown at the top of \Cref{IDLscalarPart} in the
    \texttt{KernelFunction} IDL specification.
    The variables \texttt{inner} and \texttt{outer} define the scope of the
    kernel function.
    More specifically, all sources of data outside \texttt{outer} are masked out
    and \texttt{inner} is considered as the control origin.
    Typically, these variables are either identical, or \texttt{outer} captures
    the outermost loop of a loop nest, while \texttt{inner} caputers the
    innermost.

    In line 2, all incoming control flow to \texttt{inner} is identified, which
    is then used in the pivotal generalised dominance constraint in line 23.
    The previous example had the variables {\tt precursor} and {\tt backedge}
    fill this role.
    Lines 3--11 collect all the sources of data from outside the scope of the
    kernel function that are used within it.
    This corresponds to the array \texttt{outside} in the previous example and
    is also used in line 23.
    Finally, lines 12--19
...
( collect i 4 {origin[i]} has control flow to {inner} and
  collect i 24 ( inherits LocalConst
                 ...
                 {constants[i].value} has
                     data flow to {constants[i].use} and
                 {inner} control flow
                     dominates {constants[i].use}) and
  collect i 8 ( {outer} control flow
                    dominates {restrictions[i].value} and
                {restrictions[i].value} strictly
                    control flow dominates {inner} and
                {restrictions[i].value} has data flow to
                    {restrictions[i].use} and
                {inner} control flow
                    dominates {restrictions[i].use}) and

  all flow from {restrictions[0..8].value} ([or]) any origin
...
\begin{lstlisting}[language=IDL,label={IDLscalarPart},caption=
   {IDL specification of a scalar reduction within a complex reduction and
    ...
    Its input \texttt{kernel.input} is composed of \texttt{read\_value} and the
    reduction value of the previous iteration, concatednated with
    \texttt{Concat}.
    \parfillskip=0pt}]
Constraint ScalarPart
( {kernel.output} reaches phi node
      {old_value} from {loop.end} and
  {old_value} strictly control flow
      dominates {loop.end} and
  ...
  inherits KernelFunction
      with {begin}      as {outer}
       and {loop.begin} as {inner} at {kernel} and
