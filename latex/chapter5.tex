    ...
    from reimplementing peephole optimisations to detecting code that is
    suitable for polyhedral analysis.
    This chapter extends this language to the Idiom Description Language (IDL)
    and uses it to implement novel detection functionality for the automatic
    ...
    Relevant loops contain indirect memory access and reduction operators
    with non-trivial control flow.
    Complex reduction and histogram computations were previously not
    interpreted as a single class of calculations.
    However, this chapter demonstrates that several standard benchmark programs
    are dominated by such computations.

    After introducing the {\em computational idiom} of complex reductions and
    histograms, their specification with constraints is investigated.
    IDL is specified as an extension to CAnDL, with additional language features
    to capture well-behaved kernel functions.
    Such kernel functions are an integral component of reductions, where they
    model the reduction operator.
    However, this extends also to many other important computational idioms,
    such as stencil codes.
    
    By grouping complex reductions and histogram computations together and
    formulating them in IDL, a completely automatic detection in C/C++ code is
    possible.
    Parallelisation routines were implemented to complement the detection.
    Established parallelisation methods for reductions then resulted in good
    speedups on several relevant programs.
    This was evaluated on the NAS Parallel Benchmark suite, the Parboil
    benchmarks, and Rodinia.
...
\begin{lstlisting}[language=IDL]
Constraint ([\bfseries KernelFunction])
( collect i  4 ( {entries[i]} has control
                     flow to {scope.begin}) and
  collect i 24 ( inherits ([\bfseries LocalConst])
                     with {scope} as {scope}
                                  at {outside[i]} and
                 {outside[i].value}
...
\begin{lstlisting}[language=IDL,label={IDLscalarPart},caption=
   {IDL specification of a scalar reduction within a complex reduction and
    ...
    \texttt{Concat}.
    \parfillskip=0pt}]
Constraint ([\bfseries ScalarPart])
( {kernel.result} reaches phi node
      {old_value} from {loop.end} and
  inherits ([\bfseries ScopeValue])
      with {loop}      as {scope}
  ...
  inherits ([\bfseries KernelFunction])
      with {loop} as {scope} at {kernel} and
  inherits ([\bfseries Concat])(N1=31,N2=1)
...
Constraint ([\bfseries HistoPart])
( inherits ([\bfseries ConditionalReadModifyWrite])
      with {loop}              as {scope}
...
Constraint ([\bfseries ComplexReductionsAndHistograms])
( inherits ([\bfseries For]) at {loop} and
  collect k 32 ( inherits ([\bfseries VectorRead])
                     with {loop.iterator}  as {input_index}
                      and {read_values[k]} as {value}
                      and {loop}           as {scope}
                                           at {read[k]}) and
  collect k  2 ( inherits ([\bfseries HistoPart])
                     with {loop.begin}  as {begin}
                      and {read}        as {read}
                      and {loop}        as {loop}
                      and {read_values} as {read_values}
                                        at {histo[k]}) and
  collect k  2 ( inherits ([\bfseries ScalarPart])
                     with {loop.begin}  as {begin}
                      and {loop}        as {loop}
                      and {read_values} as {read_values}
                                        at {scalar[k]}) and
  ...
  inherits ([\bfseries SideEffectFreeCalls])
      with {loop} as {scope})
End
\end{lstlisting}
\end{figure}
