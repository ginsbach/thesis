...
The reduction idiom occurs widely in numerical applications \cite{hall1996maximizing}.  This
includes standard HPC workloads based on linear algebra as well as
embedded benchmarks and emerging machine learning and computer vision
applications \cite{Reddy2016Reduction}.
...
example, the sum of a set of floating point numbers.  These scalar
reductions are prevalent but rarely constitute performance
bottlenecks.  There is a larger class of reduction,  sometimes referred
to as irregular reductions or histograms \cite{Han:2000:ILA:645678.663936}.
Such reductions are typically more intense computationally and can be
more profitably exploited.

%% In this paper we want to draw the
%% attention to general histogram reductions that we recognise as
%% performance critical in several benchmark programs and that can be
%% exploited for automatic compiler parallelisation.

Discovering and exploiting scalar reductions in programs has been
studied for many years \cite{pottenger1995idiom}.  The treatment of
more general reduction operations has received  less attention.
% than systematic approaches
%to scalar reductions and to do-all parallelism detection. 
%%  As we show
%% in this paper, there are many reduction opportunities within programs
%% that go currently undetected and this includes in particular those
%% reductions that are most critical to runtime performance.
...
\cite{Gutierrez:2000}, examining trade-offs in implementation \cite{yu2006adaptive}
 or exploitation of novel hardware \cite{Huo2011HiPC}. Early work
focused on well structured Fortran and paid little attention to
...
In this paper we develop a new compiler based approach that
...
Rather than developing a bespoke detection pass, we introduce a novel
constraint based 
representation of generalized reductions. This is embedded in a constraint 
language which allows easy extensions to cover other idioms 
and has  been implemented as an LLVM \cite{lattner2004llvm}  pass
after lowering to SSA-form.  
%% This formulation allows us to specify the
%% reduction idiom in a semi-formal way using data flow and control flow
%% constraints.
...
%% With the advent of GPUS and heterogeneous systems, reductions can be efficiently
%% exploited and can lead to significant additional performance improvement over
%% standard parallelisation. 

%Once all candidate reductions have been identified we then use a specialised SAT
%solver which by a careful enumeration strategy reduces the theoretic
%combinatorial complexity of detection to linear time in practice.

We applied our approach to C/C++ versions of 3 well known benchmark
...
Figure \ref{sum-figure} shows a standard scalar reductions.  Rather
...
\begin{figure}[h]
\centering
\includegraphics[width=0.45\textwidth]{figures/nicepicture.png}
...
\begin{figure}[h]
\centering
 \includegraphics[width=0.45\textwidth]{figures/nicepicture2.png}
...
%% In the motivation section we described how reductions can be
%% parallelized and suggested that the same basic technique can be
%% applied to a wide range of generalised reductions.  To enable
%% compilers to exploit this, we need a precise definition of the kinds
%% of computations that fit this approach.  We will start with an
%% intuitive description and then deconstruct it into a specification
%% that is made up of constraints that can be automatically verified.
...
%% are precise enough for human readers but we need a
%% more formal definition of these properties to implement functionality
%% that can automatically verify them. 

\begin{figure}[th]
{\small
A for loop is a tuple
\begin{align*}
(&\text{\tt entry, exit, loop begin, loop jump, test,}\\
 &\text{\tt loop body, backedge, iterator, next iter,}\\
 &\text{\tt iter begin, iter step, iter end})\\
 &\in\text{\tt LLVM::Value}^{12}
\end{align*}
such that the following holds:
\begin{align*}
&\texttt{entry}\xrightarrow[cfg]{sese}\texttt{exit}\mathrel{\land}\\
&\texttt{entry}=\texttt{branch}(\texttt{loop begin})\mathrel{\land}\\
&\texttt{loop jump}=\texttt{branch}(\texttt{test}, \texttt{loop body}, \texttt{exit})\mathrel{\land}\\
&\texttt{loop body}\xrightarrow[cfg]{sese}\texttt{backedge}\mathrel{\land}\\
&\texttt{backedge}=\texttt{branch}(\texttt{loop begin})\mathrel{\land}\\
&\texttt{loop jump}\xrightarrow[cfg]{dominate}\texttt{exit}\mathrel{\land}\\
&\texttt{test}=\texttt{int comparison}(\texttt{iterator},\texttt{iter end})\mathrel{\land}\\
&(\texttt{iter end}\in\texttt{constant}\mathrel{\lor}\\
&\texttt{iter end}\xrightarrow[cfg]{dominate}\texttt{entry})\mathrel{\land}\\
&\texttt{iterator}=\Phi(\texttt{next iterator},\texttt{iter begin})\mathrel{\land}\\
&(\texttt{iter begin}\in\texttt{constant}\mathrel{\lor}\\
&\texttt{iter begin}\xrightarrow[cfg]{dominate}\texttt{entry}))\mathrel{\land}\\
&\texttt{next iter}=\texttt{add}(\texttt{iterator},\texttt{iter step})\mathrel{\land}\\
&(\texttt{iter step}\in\texttt{constant}\mathrel{\lor}\\
&\texttt{iter step}\xrightarrow[cfg]{dominate}\texttt{entry}).
\end{align*}}
\caption{Constraint Formulation of For Loops}
\label{for-loop-constraints}
\end{figure}

%% The detection of loops in LLVM IR is of course a solved problem and LLVM has compiler passes for this purpose built in.
%% Therefore it might seem pointless to explicitely state these properties again in this context.
%% The algorithm that we will introduce is however much more flexible and can detect any computational idiom that can be formulated in this specification language.
...
%% for this purpose that can also be used 
%% to specify computational idioms such
%%as scalar and histogram reductions. %%  Computational idioms are defined
%% as a combination of constraints on LLVM IR values and the underlying
%% data and control flow graphs.
%The separation of the detection algorithm and the specification of the idiom 
% allows for a very modular and extensible system.
...
%% We now have a well defined specification of reductions.
%% In the next section we will derive an algorithm that automatically finds parts of LLVM IR code in a given function that correspond to these specifications.
...
functionality in the LLVM framework. %%  Such a generic algorithm can
%% then also be used to detect other computational idioms, e.g.\ linear
%% algebra or stencil computations.

%We now introduce some mathematical formalities.
...
%% We want to construct an algorithm that can spot idioms such as
%% reductions in single static assignment IR code that are specified as
...
This, however, is exponential in the complexity of the specification.
...
\begin{algorithmic}[1]
\Procedure{Detect}{$\mathcal F$, $k\gets 0$, $x\gets\emptyset$}
\If{$k = n$}
\State\Call{Yield}{x}
\Else
\State$Y\gets\{y\in\mathcal F\mid c_{k+1}(x_1,\dots, x_k, y)=\text{true}\}$
\For{$y\in Y$}
\State \Call{Detect}{$\mathcal F$, $k+1$, $(x_1,\dots,x_k,y)$}
\EndFor
\EndIf
\EndProcedure
\end{algorithmic}
...
This has transformed our problem into a depth first tree traversal on the search for leaves that have distance $n$ to the root.
...
%% Starting instead with the begin of the loop body on the other hand is
%% a bad idea.  Without any context, each basic block could be the start
%% of a loop body and hence this would result in a lot of dead ends being
%% taken in the tree
.
...
%% Figure \ref{code-snippet} shows an example of how the constraint specifications are integrated into C++.
%% We have considered external files for constraint specifications to allow adding specifications without recompilation.
%% This might be implemented in future work.
...
the original 8 NAS benchmarks plus two of the newer unstructured components UA and DC. %{\em MOB why not BT-IO, DT}
...
%{\em MOB check number here are correct}
%{\em MOB Any programs missing and if so - why??}
...
We compiled the sequential versions of the benchmark programs with
...
%For each of the
%reductions that our detection pass identified, 
...
Aside from reductions, we also formulated dense matrix multiplication
as a set of constraints that could be detected and replaced with a library call
% and sparse matrix vector multiplications to
...
structures.  Figures  \ref{npb_scops},\ref{parboil_scops},\ref{rodinia_scops}
shows the number of static control-flow
regions SCoPs that baseline Polly finds. As the reduction pass of
...
%We use a extra code transformation pass that uses a formulation of matrix multiplication in our constraint language and replaces occurences with calls to OpenBLAS.}
