...
\section{Moving on the Spectrum of Specialisation}
    ...
    input arbitrary C/C++ code and then recognises whether parts of the code are
    actually expressible in the more restrictive polyhedral model
    ...
    \citep{Moll:2016:ISS:2892208.2892217,Doerfert2015Polly}.
    This confirms the predictions of \Cref{sec:hostkernel}.

\subsection{Contributions of this Thesis}

    This thesis derives a new approach for automatically recognising constrained
    program sections using a generic approach that is not tied to a particular
    model.
    The approach emerges from the analysis of a common program model that host
    ...
    with their most interesting features based on graph and list structures.
    ...
    representations.
    The use of constraint solver techniques then make an automatic detection of
    ...
    that can express many different such models, and a corresponding toolchain
    is developed that uses these specifications to automatically detect
    corresponding code sections.
    The polyhedral model can be formulated in this system, but also stencil
    computations, sparse and dense linear algbera, complex reduction and
    histogram computations and many more.
