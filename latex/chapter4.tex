    ...
    Optimizing compilers use sophisticated program transformations to exploit
...
\begin{figure}[b]
\centering
\caption{Declarative CAnDL Specificaiton}
\label{fig:candlspec}
...
\end{figure}
    ...
    To give an example of the CAnDL workflow, consider the algebraic equation
    in \autoref{fig:root}, based on the mathemtical definition of the square
    root.
    Assuming an environment without the particularities of floating point
    arithmetic (i.e. assuming the \texttt{-ffast-math} flag is set), the
    compiler could use this equality to eliminate some square root invocations
    in user code.
    This is desirable, as the square root function has to be approximated with
    relatively expensive numerical methods.
    \begin{align}
    \label{fig:root}
    \forall a\in \mathbb{R}\colon\ \sqrt{a*a}=|a|
    \end{align}

    More specifically, the compiler should use the equation left to right:
    It should analyse the user code in order to find the left side of the
    equation and then transform all those occurences into the right side of
    the equation.
    The compiler must detect occurrences of $\sqrt{a*a}$ in the LLVM IR code and
    replace them with a call to the \texttt{abs} function.
    ...
    \citet{Yang:2011:FUB:1993316.1993532,Menendez:2017:ADP:3062341.3062372}, it
    inevitably leads to buggy code.

    Instead, CAnDL allows a declarative description of the analysis problem,
    which is then automatically turned into C++ code implementing the required
    functionality.
    The specific CAnDL code is shown in \autoref{fig:candlspec}.
