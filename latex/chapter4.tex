...
\begin{tabular}{|c|}
    \hline
    $formula ::= \text{\it atomic}\mid\text{\it conjunction}\mid\text{\it disjunction}\mid\text{\it conRange}\mid \text{\it disRange}\mid\text{\it include}\mid\text{\it collect}$\\
    \hline
\end{tabular}
\end{figure}
...
\begin{lstlisting}[language=CAnDL, label=localconstant, caption=
   {This specification restricts {\tt value} to remain constant during the
    ...
    variable that needs to be constrained in this way.}]
...
\end{figure}
    ...
    Also, {\it conRange} and {\it disRange} statements are lowered to
...
\subsubsection{Experimental Setup}
    ...
    This timing includes all the other passes within LLVM plus the CAnDL
    code path.
...
\begin{lstlisting}[language=CAnDL, label=polyhedralCAnDL, caption=
   {Fragments from the specification of Scalar Control Parts (SCoPs) in CAnDL:
    SCoPs are defined in lines 1--11 by applying multiple restrictions to the
    containing loop.
...
\section{Conclusions}
    ...
    This enables the capturing of entire hot loops at a time.
