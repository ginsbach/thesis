...
\end{lstlisting}
\caption{C++ code generation: The code is generated to first instantiate
         atomic constraints, then compose higher-level constructs and
         finally assemble a backtracking solution for the solver.
         \parfillskip=0pt}
\label{fig:codegen}
\end{figure}
...
\end{lstlisting}
\caption{Factorisation opportunities in CAnDL: This captures
         some opportunities that LLVM {\tt instcombine} misses. {\tt SumChain}
         and {\tt MulChain} are themselves specified in CAnDL (16 LoC).}
\label{fig:facopport}
\end{figure}
...
Detected in Rodinia & 24 & 24 + 4\\ \hline
...
End
\end{lstlisting}
\caption{CAnDL code for multiplication chains involving genuine vectors and
         hoisted scalars:
         After separating the two cases, some of the multiplications can be
         performed on scalars instead.\parfillskip=0pt}
\label{fig:Lewis}
\end{figure}
