...
vector<pair<string,BacktrackingPart>> result(3);
...
\end{figure}
    ...
    CAnDL program, so \texttt{select<1>} is used.
...
\subsection{The Solver}
    ...
    This class wraps around the class \texttt{BacktrackingPart} that is defined in
...
\end{lstlisting}
\vspace{-0.3cm}
\caption{The BacktrackingPart interface.}
...
\end{figure}
    ...
    \texttt{resume} allow implementations of \texttt{BacktrackingPart}
    to do bookkeeping.

    Pseudocode for the backtracking constraint solver is shown in
    \autoref{fig:solver}.
    The array of \texttt{BacktrackingPart}s is named \texttt{atom}, \texttt{solution}
...
\end{figure}
    ...
    implementation of the \texttt{BacktrackingPart} interface for the different atomic
    constraints and their interactions.
    For constraints like the \textbf{is an add instruction} statement,
    this is straightforward.
    The member function \texttt{skip\_invalid} only has to test whether
...
\begin{figure}[ht]
\begin{lstlisting}
class AddInstruction : public BacktrackingPart {
public:
...
\end{lstlisting}
\vspace{-0.3cm}
\caption{BacktrackingPart for additions}
