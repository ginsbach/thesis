...
\begin{figure}[t]
  ...
  \begin{tabular}{|c|c|}
    ...
    \hline
    \hline
    $\text{\it variable}\text{\bf.args[}\left<\text{\bf n}\right>\text{\bf]}\ \text{\bf =}\ \text{\it variable}$ & $(n,y,x)\in DFG_\mathcal F$\\
    $\text{\it variable}\ \text{\bf $\in$}\ \text{\it variable}\text{\bf .args}$ & $(x,y)\in DFG_\mathcal F^*$\\
    \hline
    $\text{\it variable}\text{\bf.successors[}\left<\text{\bf n}\right>\text{\bf]}\ \text{\bf =}\ \text{\it variable}$ & $(n,x,y)\in CFG_\mathcal F$\\
...
\subsubsection{Additional Atomic Constraints}
    ...
    The set of atomic constraints that CAnDL supports is easily extensible.
    Some possible additions include constraints on function attributes and value
...
\end{figure}
    ...
    and disjunction constructs, there are range based versions that operate on
    arrays of variables:
\begin{figure}[h]
  \centering
  \begin{tabular}{|c|}
...
\end{figure}
    ...
    formula together.
    Otherwise identical, the {\it forany} construct invokes logical
    disjunctions ($\lor$) instead.

    Although variable identifiers are black boxes to atomic constraints, their
    syntactic structure is clearly relevant to range expressions.
    Variable identifiers are constructed as follows:
...
\subsubsection{Collect}
    ...
    of a given formula in variables.
    It is used to implement constraints that require the logical
...
\end{figure}
    ...
    \end{align*}
    The following equivalences can now be applied:
    \begin{align*}
        S_1\subset S_2\iff S_1 = S_1\cap S_2\iff S=S_1\land S=S_1\text{ and }S_2\text{ for some set }S.
    \end{align*}
    This means that if a set $S$ is constrained to be equal to both $S_1$ and
    $S_1\cap S_2$ at the same time, the constraints are satisfiable if and only
    if the implication in \Cref{fig:implication} holds.
    ...
    Note that only the values that qualify for \texttt{ins} were of interested
    initially.
    The range of variables \texttt{arg} was only introduced to further restrict
    \texttt{ins}, not because its contents are necessarily of interest
    themselves.
...
\end{figure}
    ...
    (i.e.\ all \texttt{load} and \texttt{store} instructions) and then use
...
\subsection{Expressing Larger Structures}
    ...
    This is expressed in \Cref{fig:localconstant}.
...
\section{Implementation}
    ...
    CAnDL interacts with the LLVM framework, as shown in \Cref{fig:build2}.
