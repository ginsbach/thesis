...
\subsection{Constraint Programming for Program Analysis}
    ...
    There are other advanced approaches to extracting high-level structures
    program program using constraints and verification systems.
    \cite{Mendis2015Helium, Kamil2016Verified} use temporal logic as the
    ...
    to find provably correct translations into the high-level description
    language Halide.
    This high-level language then specialises the code again, exploring the
    ...
    model many interesting computational idioms, such as sparse linear algebra.
...
\subsection{Delarative Programming Languages for Program Analysis}
    ...
    Further approaches to generating compiler optimisations from specification
    languages that focus on formal verification instead of programmer
    productivity include Rhodium
    \citep{Lerner:2005:ASP:1040305.1040335} and Gospel
...
\section{Computational Idioms}
    ...
    The concrete concepts are partially overlapping and can be surprisingly
    vague.
    ...
    many different domains with related concepts.
...
\subsection{Higher Order Functions}
    ...
    In functional programming languages, the natural representation of
    common programming patterns are higher order functions.
    Such functions take other functions as parameters.

    Higher order functions can be used to describe many common computational workloads.
    The popularity of the MapReduce framework \citep{Dean2008MapReduce} for example stems from the observation that many
    big data workloads exhibit characteristics that can be expressed efficiently using a combination of the higher order
    functions \texttt{map} and \texttt{reduce} (another name for \texttt{fold}).
    It provides an idiom based approach to the development of big data applications, enabling shorter development times and
    more predictable performance.
    The inspiration for this framework came from the observation that much of the existing, individually programmed
    software algorithms at Google implicitly used similar programming models already.

\subsection{Parallel Dwarfs}

    The so called Berkeley Dwarfs are a collection of 13 computational methods
    that together comprise a large portion of the common parallel computing
    workloads \citep{Asanovic06thelandscape}.
    Each Dwarf is a computational pattern that is common in important
    applications and has persisted more or less unchanged for many years.
...
\label{alg-skel-section}
Another concept that can be considered a subset of computational idioms is the notion of ``Algorithmic Skeletons''
\citep{Cole1991Algorithmic}.
It was introduced as a way to classify the behaviour of parallel programs according to their organisation of workload
distribution.
This is mostly done with the motivation to introduce new, more higher level programming models and tools for parallel
programming.
One major inspiration to the concept was the lack of functionality in mainstream programming languages to equal the well
established higher order functions of functional programming languages, such as the \textbf{map} function to express
element wise application of a single function to an array.

Among the ``Algorithmic Skeletons'' are \textbf{Fixed Degree Divide \& Conquer} and the \textbf{Task Queue}.
The concept of ``Algorithmic Skeletons'' has been used to implement programming frameworks and libraries, including
Skandium \citep{Leyton2010Skandium}, Eden \citep{Loogen2005Parallel}, eSkel \citep{Cole2004Bringing} and SkelCL
\citep{Steuwer2011SkelCL}.
Furthermore, the Intel Thread Building Blocks library \citep{Reinders2007Intel} is based on this concept.

As opposed to the higher order functions used in functional programming and similarly to the Berkeley Dwarfs,
the definitions for algorithmic skeletons are informal and not intended for automated reasoning.
They are however heavily inspired by higher order functions and similarly describe the algorithmic structure of
computations.
This distinguishes them from the Berkeley Dwarfs, which are more focused on mathematical domains.
