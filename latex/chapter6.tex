...
\subsection{Accelerating Sparse Linear Algebra}
    ...
    The structure of this computation has several features that make it
    unsuitable for most established compiler optimisations:
    ...
    IDL can express this sparse idiom
    (\Cref{sec:idioms}, \Cref{csr_lilacwhat_fig}).
    The LLVM IR code, together with the IDL program, is input to a constraint
    solver,  which outputs a constraint solution as shown in
    \Cref{fig:spmvexample2}.
    Different parts of the IR have been assigned to IDL variables.
    ...
    In the next section, the formulation of linear algebra and stencils in the
    Idiom Detection Language is derived in detail.
...
\end{lstlisting}
\begin{lstlisting}[language=IDL,
                   label={fig:spmv},caption={IDL specification of sparse matrix vector}]
Constraint SPMV
( inherits For and
