...
\label{sec:idiomresults}
    ...
    are reported.
    Detailed results capture the performance of each targeted backend interface.
...
\subsection{Idiom Detection}
    ...
    Polly and ICC were only capable of detecting simple scalar reductions, but
    we are able to detect histogram reductions, {\em e.g.} in
    the \emph{histo} benchmark as well.
    For stencils, Polly detects two in \emph{lbm} and \emph{stencil} while our
    approach detects all the stencils in \emph{lbm}, \emph{stencil} and
    \emph{MG}.
