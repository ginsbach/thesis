...
\subsubsection{Jagged Diagonal Storage}
    ...
    \autoref{jds_lilacwhat_fig} shows this on the example sparse matrix from
    \autoref{csr_lilacwhat_fig}, and derives the corresponding IDL code for the
    {\tt SPMV\_JDS} idiom underneath.
...
\section{Not Syntactic Pattern Matching}
    ...
    For example, there are two syntactically distinct programs in
    \Cref{fig:gemmexamples}, which nevertheless are both implementations of
...
\end{figure}
    ...
    usual LLVM IR representation, {\em e.g.} SIMD compiler intrinsics, would
