...
\subsection{IDL Example}
    ...
    At the core of our approach is IDL, which is described in \Cref{sec:idl}.
...
\begin{figure}[t]
\begin{lstlisting}[language=IDL,escapechar=|,
...
\end{figure}
    ...
    understandable program (\Cref{fig:IDLfactorization}).
    ...
    From the formulation in \Cref{fig:IDLfactorization}, the IDL compiler builds
    ...
    \Cref{fig:firstexample} shows a simple example.
...
\end{lstlisting}
\begin{lstlisting}[language={},captionpos=t,title=
...
\end{figure}
    ...
    \Cref{fig:IDLfactorization} say that {\tt factor} can either be the first
    ...
    \ref{line:factorSecondRight} of \Cref{fig:IDLfactorization} say that
...
Therefore, the solution at the bottom of \Cref{fig:firstexample} is the only factorization opportunity in the code.
...
\section{Basic Control Flow Structures}
    ...
    \Cref{candl_sese} shows the specification, which contains some
...
( {precursor} is branch instruction and
  ...
  {end} control flow post
        dominates {begin} and
  {precursor} strictly control flow
              dominates {begin} and
  {successor} strictly control flow post
              dominates {end} and
...
\section{Loop Structures}
    Building on top of the SESE region, \Cref{candl_loop} presents the CAnDL
