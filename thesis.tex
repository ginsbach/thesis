...
\lstdefinelanguage{IDL}{
 morekeywords = { with, collect, for, and, or, at, as,
                  if, then, else, endif, not, otherwise, specified},
...
\abstract{
    The scaling limitations of multi-core processor development have lead to a
    diversification of the processor cores used within individual computers.
    Heterogeneous computing has become widespread, meaning the cooperation of
    ...
    parallel computations unrelated to computer graphics.
    ...
    In particular, heterogeneous systems are a challenge for compilers, which
    ...
    specification and automatic detection of {\t computational idioms} within
    ...
    reductions and histogram computatoins.
    ...
    The constraint programming methodology is derived in several steps,
    eventually resulting in the design and implementation of the
    Compiler Analysis Description Language (CAnDL), and the extended
    ...
    The pipeline is evaluated on established benchmark collections NPB and
    Parboil.
    The approach was effective on 10 of the benchmark programs, resulting in
    significant speedups of 1.26$\times$ on ``histo'' to 275$\times$ on
