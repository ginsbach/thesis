...
\abstract{
    The scaling limitations of multi-core processor development have resulted
    in a diversification of the processor cores used within individual
    computers.
    Heterogeneous computing has become widespread, meaning the cooperation of
    several structurally different processor cores.
    Central processor cores are most frequencty complemented with graphics
    processors, which despite their name are suitable for many computations
    unrelated to computer graphics.
    Furthermore, deep learning accelerators are rapidly gaining relevance.

    Many applications could profit from heterogeneous computing, but are
    inhibited by the surrounding software ecosystems.
    Heterogeneous systems are a challenge for the established compilers, which
    often only target the increasingly marginalised homogeneous CPUs.
