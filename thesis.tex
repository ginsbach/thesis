...
\abstract{
    ...
    many applications to utilise heterogeneous paralellism, including
    graphics processors and increasingly also deep learning accelerators.
    This is a challenge for existing software ecosystems.
    Compilers can map programs onto homogeneous processors efficiently, but
    heterogeneous acceleration is often only accessible via libraries and domain
    specific languages.
    This requires application rewrites, and the constantly changing hardware
    landscape challenges the achievements in portability  and longevity of
    programs that are taken for granted in modern computing.
    ...
    The contribution of this thesis is a constraint programming methodology to
    define and then automatically detect algorithmic structure in compiler
    intermediate representaiton.
    This can be used to recognise narrow interfaces within programs that are
    written in general purpose programming languages such as C++.
    ...
    Once a part of a program has been specialised, the general purpose compiler
    can reformulate it in a more restrictive model and pass it on to specialised
